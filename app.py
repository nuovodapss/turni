...
def make_excel_bytes(sc: Scenario) -> bytes:
    ...
    with pd.ExcelWriter(out, engine="openpyxl") as writer:
        ...
        params.append(("Peso adiacenza lavoro", sc.weights.w_adj_work))
        params.append(("Peso tripla lavoro", sc.weights.w_triple_work))
...
with tabs[0]:
    ...
    with w1:
        sc.weights.w_change = st.slider("A) Repair: cambia meno possibile", 0, 2000, int(sc.weights.w_change), step=10)
    with w2:
        sc.weights.w_adj_work = st.slider("B) Evita consecutivi (adiacenze)", 0, 200, int(sc.weights.w_adj_work), step=1)
    with w3:
        sc.weights.w_triple_work = st.slider("B2) Evita run (triple)", 0, 200, int(sc.weights.w_triple_work), step=1)
    with w4:
    ...
    sc.weights.w_hours_dev = st.slider("E) Debito orario (oltre tolleranza)", 0, 200, int(sc.weights.w_hours_dev), step=1)
