...
class ObjectiveWeights:
    # A) mantenere matrice (repair)
    w_change: int = 1000
    # B) penalità adiacenza lavoro-lavoro (riduce consecutivi)
    w_adj_work: int = 15
    w_triple_work: int = 25
    # C) equità notti (range max-min)
    w_balance_nights: int = 30
    # D) equità weekend/festivi (range max-min)
    w_balance_weekend: int = 20
    # E) ore target (deviazione oltre tolleranza)
    w_hours_dev: int = 10
...
def build_demo_scenario() -> Scenario:
    ...
    for i in range(12):
        nurses.append(
            Nurse(
                ...
                target_ore_periodo=140,  # esempio: 20 giorni * 7h
    ...
    sc = Scenario(
        ...
        end_date=end,
        nurses=nurses,
        coverage=CoverageRules(min_M=2, min_P=2, min_N=1, weekend_min_M=2, weekend_min_P=1, weekend_min_N=1),
        rules=RuleParams(
        ...
        weights=ObjectiveWeights(w_change=1000, w_adj_work=15, w_triple_work=25, w_balance_nights=30, w_balance_weekend=20, w_hours_dev=10),
    )
    sc.ensure_matrices()
